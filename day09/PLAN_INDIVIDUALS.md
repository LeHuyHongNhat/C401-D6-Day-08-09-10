...
```python
# graph.py — AgentState (full schema, khớp với code thực tế)
...
class AgentState(TypedDict):
    ...
    needs_tool: bool                 # True nếu cần gọi MCP tools

    # Worker outputs
    retrieved_chunks: list           # output của retrieval worker [{text, source, score}]
    retrieved_sources: list          # danh sách source files (dùng trong eval_trace.py)
    policy_result: dict              # output của policy worker
    mcp_tools_used: list             # danh sách MCP tools đã gọi

    # Final
    final_answer: str
    sources: list                    # sources có trong answer
    confidence: float
    hitl_triggered: bool

    # Trace & history
    history: list                    # lịch sử các bước đã chạy
    workers_called: list             # danh sách workers đã được gọi (bắt buộc trong grading JSONL)
    latency_ms: Optional[int]        # thời gian xử lý (ms) — được tính trong run_graph()
    run_id: str                      # ID của run này — dùng đặt tên trace file
```
...
- ✅ `AgentState` định nghĩa đầy đủ (kể cả `workers_called`, `retrieved_sources`, `latency_ms`, `run_id`)
- ✅ LangGraph `StateGraph` với conditional edges — `python graph.py` chạy không lỗi
- ✅ Supervisor route đúng cho 3 loại task (SLA → retrieval, refund → policy, access → policy)
- ✅ `worker_contracts.yaml` publish (v2.0 — đủ Supervisor + 3 Workers + MCP server với 4 tools)
- ✅ Trace JSON lưu vào `artifacts/traces/run_[ID].json` có đủ fields
...
[NHat][S1] implement supervisor routing + LangGraph StateGraph
[NHat][S1] connect full graph + smoke test 3 queries ✅
...
> **Lưu ý:** `workers/retrieval.py` đã có skeleton hoàn chỉnh (207 dòng). Khánh không cần viết từ đầu, chỉ cần:

**Task 1 — Implement `retrieve_dense()` (skeleton đã có, cần điền logic ChromaDB):**

```python
def retrieve_dense(query: str, top_k: int = DEFAULT_TOP_K) -> list:
    embed = _get_embedding_fn()        # đã implement sẵn
    query_embedding = embed(query)
    collection = _get_collection()     # đã implement sẵn
    results = collection.query(
        query_embeddings=[query_embedding],
        n_results=top_k,
        include=["documents", "distances", "metadatas"]
    )
    # Format thành list of {text, source, score, metadata}
    # → công thức: score = round(1 - distance, 4)
    ...
```

**Task 2 — Build ChromaDB index từ `data/docs/` (nếu chưa có):**
...
for fname in os.listdir('./data/docs'):
    with open(f'./data/docs/{fname}') as f: text = f.read()
    chunks = [text[i:i+500] for i in range(0, len(text), 400)]
    for j, chunk in enumerate(chunks):
        col.add(documents=[chunk], ids=[f'{fname}_{j}'],
                metadatas=[{'source': fname, 'chunk_id': j}])
print('Index built:', col.count(), 'chunks')
"
```

**Task 3 — Test worker độc lập (KHÔNG cần graph):**
```python
...
class MockMCPServer:
    """
    Mock MCP Server — cung cấp 4 tools (theo worker_contracts.yaml v2.0):
    ...
    def dispatch_tool(self, tool: str, input_data: dict) -> dict:
        """Dispatch to the right tool. Returns structured result with timestamp."""
        timestamp = datetime.now().isoformat()

        dispatch = {
            "search_kb": self._search_kb,
            "get_ticket_info": self._get_ticket_info,
            "check_access_permission": self._check_access_permission,
            "create_ticket": self._create_ticket,
        }
        fn = dispatch.get(tool)
        result = fn(**input_data) if fn else {"error": f"Unknown tool: {tool}"}
    ...
    def list_tools(self) -> list:
        """Return list of available tool names (MCP convention)."""
        return ["search_kb", "get_ticket_info", "check_access_permission", "create_ticket"]
```
...
#### Sprint 3 (60 phút) — Implement đủ 4 tools + integrate vào policy_tool.py

**Ưu tiên #1 — `check_access_permission` (cần cho gq03 + gq09 — 26 điểm):**

```python
def _check_access_permission(self, access_level: int, requester_role: str, is_emergency: bool = False) -> dict:
    """Tool 3: Kiểm tra quyền truy cập dựa trên access_control_sop.txt"""
    # Level 1: self-service
    # Level 2: manager approve
    # Level 3: IT Lead + CISO approve, max 24h nếu emergency
    approval_map = {
        1: {"can_grant": True, "required_approvers": ["IT Helpdesk"], "emergency_override": False},
        2: {"can_grant": True, "required_approvers": ["Direct Manager", "IT Helpdesk"], "emergency_override": is_emergency},
        3: {"can_grant": is_emergency, "required_approvers": ["IT Lead", "CISO"], "emergency_override": is_emergency,
            "notes": ["Max 24h nếu emergency", "Phải revoke sau khi sự cố xử lý xong"]},
    }
    result = approval_map.get(access_level, {"error": f"Invalid access level: {access_level}"})
    result["source"] = "access_control_sop.txt — Section 4"
    return result
```

**Ưu tiên #2 — `get_ticket_info` (cần cho gq01 + gq05):**
...
> **Lưu ý:** Dùng đúng API của `eval_trace.py` thực tế (khác với PLAN cũ):

```python
# eval_trace.py — API thực tế
from eval_trace import run_test_questions, run_grading_questions, analyze_traces

# Chạy 15 test questions
results = run_test_questions("data/test_questions.json")

# Phân tích traces
metrics = analyze_traces("artifacts/traces/")
print(metrics)  # có routing_distribution, avg_confidence, avg_latency_ms, mcp_usage_rate

# 17:00 — chạy grading
run_grading_questions("data/grading_questions.json")
# → output: artifacts/grading_run.jsonl
```

**Trường bắt buộc phải có trong mỗi trace record (không được MISSING):**
```python
required_fields = [
    "id", "question", "answer", "sources",
    "supervisor_route",   # thiếu = -20%/câu
    "route_reason",       # thiếu = -20%/câu
    "workers_called",
    "mcp_tools_used",
    "confidence",
    "hitl_triggered",
...
### Code (deadline 18:00)
- [x] `graph.py` — `python graph.py` chạy không lỗi, route đúng 3 loại task
- [x] `graph.py` — mọi route đều có `route_reason` không phải "unknown"
- [x] `contracts/worker_contracts.yaml` — đủ Supervisor + 3 workers + MCP server 4 tools
- [ ] `workers/retrieval.py` — test độc lập được, output đúng contract (Khánh)
- [ ] `workers/policy_tool.py` — xử lý đúng ít nhất 1 exception case (Khải)
- [ ] `workers/synthesis.py` — trả về answer có citation, không hallucinate (Khải)
- [ ] `mcp_server.py` — có 4 tools (`search_kb`, `get_ticket_info`, `check_access_permission`, `create_ticket`)
- [ ] `mcp_server.py` — được gọi từ `policy_tool.py`, trace ghi `mcp_tool_called` (Tấn)
- [ ] `eval_trace.py` — chạy end-to-end 15 test questions không crash (Thành)
...
### 🏆 Bonus targets (tối đa +5 điểm — theo SCORING.md)
- [ ] `confidence` score thực tế từ ChromaDB (không hard-code 0.75) → **+1**
- [ ] gq09 đạt Full marks (16/16) + trace ghi đúng 2 workers → **+2**
- [ ] `mcp_server.py` thật dùng `mcp` library hoặc HTTP server (không phải mock class) → **+2**

---

*Phân công v2.0 — Lê Huy Hồng Nhật (Tech Lead)*  
*Cập nhật: sync `AgentState` + LangGraph + MCP 4 tools + với dữ liệu API Thành + bonus targets*  
